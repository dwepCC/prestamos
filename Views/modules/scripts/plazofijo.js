...
function listar() {
	tabla = $("#tbllistado")
		.dataTable({
			...
			buttons: [
				{
					...
					title: "Reporte de Plazo fijo",
					sheetName: "Plazo fijo",
					exportOptions: {
						columns: [1, 2, 3],
					},
				},
			...
			ajax: {
				url: "Controllers/Cuenta.php?op=listarPlazoFijo",
				type: "get",
				dataType: "json",
				error: function (e) {
...
function calcular() {
	...
	//if (parseInt(plazo) === 360) {
	var total = (cantidad * interes) / 100;
	//var total = ((Math.pow((1 + (interes / 100)), (plazo / 360))) - 1) * cantidad;
	$("#cantidadFinal").val(total);
	// }
	//alert(plazo);
}
